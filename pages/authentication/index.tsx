    ...
    return (
        ...
        <div className={styles.GlobaleContainer}>
            <div className={styles.container} id="container">
                ...
                <p className={styles.paragraph}>Protecting your account is out top priority. Please confirm your account by entring the authorization code by scannig the Qr code below</p>
                <div className={styles.inputsContainer}>
                    <input type="text" id="first" className={styles.codeInput} maxLength={1} onChange={(e:any) => handleClick(e,"sec")} />
                    <input type="text" id="sec" className={styles.codeInput} maxLength={1} onChange={(e:any) => handleClick(e,"third")} onKeyUp={(e:any) => {hendleDelete(e,"first")}}/>
                    <input type="text" id="third" className={styles.codeInput} maxLength={1} onChange={(e:any) => handleClick(e,"fourth")} onKeyUp={(e:any) => {hendleDelete(e, "sec")}}/>
                    <input type="text" id="fourth" className={styles.codeInput} maxLength={1} onChange={(e:any) => handleClick(e,"fifth")} onKeyUp={(e:any) => {hendleDelete(e,"third")}} />
                    ...
                    <input type="submit" value={`   lettere left`} className={styles.submitButton} onClick={(e:any) => {
                        ...
                        console.log(data)
                    axios.post('http://10.12.10.1:3000/2fa/authenticate',data,{headers:{'Authorization': `Bearer ${localStorage.getItem("refreshToken")}`}}
                    ).then((res) => {
                        console.log(res)
                        if (res.status === 200)
                        {
